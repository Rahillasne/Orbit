...
Retained for backward compatibility with existing tests and downstream
modules (prescriber, dashboard).  New code should use the Phase-2
detectors in ``orbit.detector.heuristic``.
"""
...
class HeuristicDetector:
    ...
    Rules applied (all configurable):

    1. Total reward below threshold
    2. Action variance below threshold (robot stuck/frozen)
    3. Consecutive steps with negative reward
    4. Episode too short (immediate failure)
    5. Episode too long (stuck in loop)
    6. Explicit ``success=False`` flag
    """
    ...
    def detect_batch(self, episodes: list[EpisodeRecord]) -> list[DetectionResult]:
        """Run detection on multiple episodes."""
        return [self.detect(ep) for ep in episodes]

    def _check_reward_threshold(
        self, episode: EpisodeRecord, metrics: dict[str, float]
    ) -> str | None:
        metrics["total_reward"] = episode.total_reward
        if episode.total_reward < self.config.reward_threshold:
            return (
                f"Total reward {episode.total_reward:.2f} below "
                f"threshold {self.config.reward_threshold:.2f}"
            )
        return None

    def _check_action_variance(
        self, episode: EpisodeRecord, metrics: dict[str, float]
    ) -> str | None:
        ...
        if mean_variance < self.config.action_variance_threshold:
            return (
                f"Action variance {mean_variance:.6f} below "
                f"threshold {self.config.action_variance_threshold:.6f} (robot may be stuck)"
            )
        return None
    ...
    def _check_consecutive_failures(
        ...
        if max_run >= self.config.consecutive_failure_steps:
            return (
                f"{max_run} consecutive steps with reward below "
                f"{self.config.min_reward_per_step:.2f}",
                best_start,
            )
        return None, None
